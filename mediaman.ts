...
class MediaManControllerImpl implements MediaManController {
	...
	removeBook(collectionIdentifier: string, bookIdentifier: string): void {
		...
		console.log(`Removing book ${bookIdentifier} which should be part 
		of collection ${collectionIdentifier}`);
        ...
        "${existingCollection.name}" updated successfully.`);
			...
			.catch((error) => {
				...
				this._view.displayErrorMessage(`Failed to save the 
        modifications made to the ${existingCollection.name}
        book collection (removal of the following book: 
        ${bookIdentifier}`);
			});
	}
}
